...
module.exports = async function (client, message) {
    ...
    // Set the client activity on a cycle
    client.user.setActivity(`${config.activityInfo.messages[0]}`, { type: config.activityInfo.type});
    setInterval(() => {
            try {
                let result = config.activityInfo.messages[Math.floor(Math.random()*config.activityInfo.messages.length)]
                client.user.setActivity(`${result}`, { type: config.activityInfo.type});
        } catch(e) {}
    }, config.activityInfo.cycleTimeinSeconds * 1000);

    const db = new QuickDB();

    // See if the Embed for creating tickets is available and if its not, make one.
    let buttonEmbed = undefined
    let postChannel = client.channels.cache.find(x => x.id == config.channel_ids.post_embed_channel_id)
    if (!postChannel) {
        ...
        if (buttonEmbed == undefined) {
			...
			await postChannel.send({ files: ["https://images.jellydev.xyz/Support.gif"] }).catch((e) => func.handle_errors(e, client, `ready.js`, null));
            const FinalEmbed = new Discord.MessageEmbed()
                .setTitle(handlerRaw.title.replace("{{SERVER}}", client.guilds?.cache.get(config.channel_ids.public_guild_id).name))
    ...
    const deleteMessagesInTicketEmbedChannel = () => {
        ...
        setTimeout(deleteMessagesInTicketEmbedChannel, 60000 * config.timeouts.embed_channel_message_delete_time_in_minutes)
