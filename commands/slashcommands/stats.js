			...
			if (interaction.options._subcommand == "top10") {
				...
				for (let TicketType of handlerKeys) {
					...
					if (typestats.totalActions == 0) {
						StatsEmbed.addField(`${TicketType} Actions (${typestats.totalActions})`, `\`\`\`No actions to display!\`\`\``)
						...
						StatsEmbed.addField(`${TicketType} Actions (${typestats.totalActions})`, `\`\`\`ml\n${LinkedStats}\`\`\``)
					}
}

					StatsEmbed.addField(`Total Actions (${totalActions})`, `\`\`\`ml\n
...
Visible Ticket Messages:  ${ticketMessagesVisibleActions}\`\`\``)
					...
					StatsEmbed.addField(`User Facts`,	`***Last Ticket Interaction:*** \`${moment(userStats.lastAction).format('MMMM Do YYYY, h:mm:ss a')}\`\n***Favourite Ticket Type:*** \`${favouriteTicketType}\`\n***Least Favourite Ticket Type:*** \`${leastFavouriteTicketType}\``)
